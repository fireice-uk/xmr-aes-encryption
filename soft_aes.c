...
__m128i soft_aesenclast(__m128i in, __m128i key)
{
	__m128i out = _mm_set_epi8(
			Sbox[hi(_mm_extract_epi16(in, 5))], //15
			Sbox[lo(_mm_extract_epi16(in, 3))],
			Sbox[hi(_mm_extract_epi16(in, 1))],
			Sbox[lo(_mm_extract_epi16(in, 6))],
			Sbox[hi(_mm_extract_epi16(in, 3))],
			Sbox[lo(_mm_extract_epi16(in, 1))],
			Sbox[hi(_mm_extract_epi16(in, 6))],
			Sbox[lo(_mm_extract_epi16(in, 4))],
			Sbox[hi(_mm_extract_epi16(in, 1))],
			Sbox[lo(_mm_extract_epi16(in, 7))],
			Sbox[hi(_mm_extract_epi16(in, 4))],
			Sbox[lo(_mm_extract_epi16(in, 2))],
			Sbox[hi(_mm_extract_epi16(in, 7))],
			Sbox[lo(_mm_extract_epi16(in, 5))],
			Sbox[hi(_mm_extract_epi16(in, 2))],
			Sbox[lo(_mm_extract_epi16(in, 0))]  //0
		);
